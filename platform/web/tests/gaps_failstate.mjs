...
const probe=()=>p.evaluate(()=>({
  ...
  whenHidden:!!($('#gapTblCard')||{}).classList?.contains('hide'),
  tlHidden:!!($('#gapTblCard')||{}).classList?.contains('hide'),
  hasRetry:!!$('#gapRetry')}));
...
ok(r.whenHidden,'no empty result card is left on screen');
...
await p.evaluate(()=>{window.__setRangeApi({stateGaps:()=>Promise.resolve({data:{
  window:{from:'2026-08-25',to:'2026-08-26',h0:0,h1:23},windows:[],gap_slots:0,states_with_gap:0,
...
ok(r.whenHidden,'a clean window collapses instead of leaving an empty card');

// 5. a real result renders the cards
await p.evaluate(()=>{window.__setRangeApi({stateGaps:()=>Promise.resolve({data:{
  window:{from:'2026-08-25',to:'2026-08-26',h0:0,h1:23},gap_slots:2,states_with_gap:1,
  excluded:[],unfilled:{hours:0,posts:0},by_hour:[[14,2]],
  windows:[['NC','2026-08-26',14,'2026-08-26',16,2,23.9,3,[['NP',3]]]]}})});GW._init=0;window.__kickGaps()});
await p.waitForTimeout(300); r=await probe();
ok(!r.whenHidden,'a real result shows the table');
