...
insert into public.service_line_map
  ...
  ('Transcarent Program Schedule', 'Transcarent', 'partner_dedicated', 'active', null, 'transcarent', true, true,
   'All non-on-call Transcarent hours group under "Transcarent". SLI demand arrives as "98point6 Transcarent" - matched via demand_match; the 98point6 prefix is display-suppressed. Consult feed carries no Transcarent rows (handle-time inputs blocked on a fresh export).'),
  ('Transcarent Program Schedule September', 'Transcarent', 'partner_dedicated', 'active', null, 'transcarent', true, true,
   'Variant calendar; groups under "Transcarent".'),
  ('98point6 Transcarent On Call', 'Transcarent On-call', 'partner_dedicated', 'active', null, 'transcarent', true, true,
   'On-call bucket, labeled exactly "Transcarent On-call" (fixed naming). Supply reported separately from hourly "Transcarent"; serves the same demand.'),
